...
def gaq_setCustomVar(index,name,value,opt_scope=None):
    ...
    """
    if opt_scope :
        c._gaq['_setCustomVar'].append( "['_setCustomVar',%s,'%s','%s',%s]" % (index,name,value,opt_scope) )
    else:
        c._gaq['_setCustomVar'].append( "['_setCustomVar',%s,'%s','%s']" % (index,name,value) )
...
def gaq_addTrans(track_dict):
    ...
    for i in ['opt_affiliation','opt_tax','opt_shipping','opt_city','opt_state','opt_country'] : # fix optionals for positioning
        if i not in track_dict:
            track_dict[i] = ''
    c._gaq['_addTrans'].append( """['_addTrans',%(order_id)s,'%(opt_affiliation)s','%(total)s','%(opt_tax)s','%(opt_shipping)s','%(opt_city)s','%(opt_state)s','%(opt_country)s']""" % track_dict)
...
def gaq_addItem(track_dict):
    ...
    for i in ['category'] : # fix optionals for positioning
        if i not in track_dict:
            track_dict[i] = ''
    c._gaq['_addItem'].append( """['_addItem',%(order_id)s,'%(sku)s','%(name)s','%(category)s','%(price)s','%(quantity)s']""" % track_dict)



def gaq_trackTrans():
    """gaq_trackTrans()- You merely have to call this to enable it. I decided to require this, instead of automatically calling it if a transaction exists, because this must be explicitly called in the ga.js API and its safer to reinforce this behavior.
...
def gaq_print():
    ...
    if c._gaq['_setDomainName']:
        if single_push:
            single_pushes.append( """['_setDomainName', '%s']""" % c._gaq['_setDomainName'] )
        else: 
            script.append( """_gaq.push(['_setDomainName', '%s']);""" % c._gaq['_setDomainName'] )

    # _setAllowLinker
    if c._gaq['_setAllowLinker']:
        if single_push:
            single_pushes.append( """['_setAllowLinker', %s]""" % ( "%s" % c._gaq['_setAllowLinker'] ).lower() )
        else: 
            script.append( """_gaq.push(['_setAllowLinker', %s]);""" % ( "%s" % c._gaq['_setAllowLinker'] ).lower() )
